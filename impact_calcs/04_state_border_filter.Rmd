...
The objective of this script is to apply the level one filter to our index and count calculations from `performance_indexing.Rmd` and `performance_counts.Rmd`, respectively. The filter here simple applies a mask to the previous calculations that only includes cells that are within state waters. The state waters shape file is made in `state_waters.Rmd`. 
...
## Calculation
The below inner loop makes an all_states_raster and all_states_df from a provided index or count results raster. The outer loop applies the inner loop to all the files saved from the index performance results of the whole US. This would have been made into a function however it is not possible to have two outputs from one function in R.
...
The below take the above loop and applies is to all the files saved from the count performance results of the whole US.
```{r}
overwrite <- TRUE
save_path <- "/home/shares/aquaculture/AOA_climate_change/performance_calcs/state_borders/"
# read in state_waters shape file - this should not change
