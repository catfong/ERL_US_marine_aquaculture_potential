...
# The contents of this script also exist in the 04_state_border_filter.Rmd.
...
# The below inner loop makes a all_states_raster and all_states_df from a provided 
# index or count results raster. The below outer loop applies the inner loop to
# all the files saved from the count performance results of the whole US.
# This would have been made into a function however it is not possible to have 
# two outputs from one function in R.

overwrite <- TRUE
save_path <- "/home/shares/aquaculture/AOA_climate_change/performance_calcs/state_borders/"
...
if (result_type == "index") {
  performance_file_path <- "/home/shares/aquaculture/AOA_climate_change/performance_calcs/mean_index/"
} else if (result_type == "count") {
  performance_file_path <- "/home/shares/aquaculture/AOA_climate_change/performance_calcs/counts/"
} else {
  stop("result_type must be index or count")
}
...
for (j in 1:n_distinct(performance_files_list)) { 
  ...
  for (i in 1:n_distinct(state_list)) { 
    ...
    # if statement to deal with the differences between count and index raster layer names
    if (result_type == "index") {
    ...
    # change the column names to include result type and scenario_timeperiod
    colnames(state_df) <- c("cell", "x", "y", 
                            paste0("all_species_", result_type, "_", scenario_timeperiod),
                            paste0("fish_", result_type, "_", scenario_timeperiod),
                            paste0("mollusc_", result_type, "_", scenario_timeperiod),
                            paste0("seaweed_", result_type, "_", scenario_timeperiod))
  ...
  terra::writeRaster(x = all_states_raster,
                     filename = paste0(save_path, "rasters/", result_type,"/", result_type,"_",scenario_timeperiod, ".tif"),
                     gdal="COMPRESS=NONE",
                     overwrite = overwrite)
  
  # save the data frame as a csv
  write.csv(x = all_states_df,
            file = paste0(save_path, "data_frames/", result_type,"/", result_type,"_",scenario_timeperiod, ".csv"))
